  ...
  return (
    <div className="space-y-6 rounded-3xl border border-white/10 bg-white/5 p-4 backdrop-blur-sm">
      <div className="flex justify-between items-center">
        <h3 className="text-sm sm:text-lg lg:text-xl font-bold">Technical Skills</h3>
        <div className="flex gap-2">
          <div className="flex gap-2">
            ...
            >
              {breakpoint !== "lg" ? <LayoutGrid className="w-4 h-4" /> : "Grid View"}
            </Button>
            ...
            <Button
              ...
              className="flex items-center gap-1 p-2 sm:p-4"
            >
              {breakpoint != "lg" ? <BarChart className="w-4 h-4" /> : "Progress View"}
            </Button>
      ...
      {!isInView && <LoadingSpinner text='Loading skills...'/> }
      {view === "progress" ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-x-6 sm:gap-y-5 auto-rows-auto">
  {activeSkills.map((skill, index) => (
    <div className="flex md:flex-row flex-col md:items-center gap-2 md:gap-4" key={`${index}-skill-${skill}`}>
      <div
