...
import { Briefcase, Computer } from "lucide-react"
    ...
    <div className="relative mx-auto max-w-4xl">
      ...
      <div className="space-y-12">
        {experiences.map((exp, index) => {
          const isItJob = exp?.technologies && exp?.technologies?.length > 0;
          return  (
            <motion.div
              key={`${index}-${exp.id}`}
            ...
            >
              ...
              <div className={`w-full md:w-1/2 ${index % 2 === 0 ? "md:pr-16" : "md:pl-16"}`}>
                <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
                  ...
                  <p className={`mb-4 text-lg font-medium ${isItJob ? 'text-purple-400' : 'text-cyan-400'}`}>{exp.company}</p>
